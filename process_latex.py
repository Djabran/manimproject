...
def remove_unnecessary_brackets_tex(tex):
    return re.sub(r"(\w+)([_^]){(\w)}", r"\1\2\3", tex)


def remove_unnecessary_brackets(expr):
    for sym in expr.free_symbols:
        match = re.search(r"[_^]{\w}", str(sym))
        if match:
            expr = expr.subs(sym, re.sub(r"(\w+)([_^]){(\w)}", r"\1\2\3", str(sym)))
    return expr


def process_sympy(tex):

    matherror = MathErrorListener(tex)

    stream = antlr4.InputStream(tex)
    lex = PSLexer(stream)
    ...
    expr = remove_unnecessary_brackets(expr)
    return expr
